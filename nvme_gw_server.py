...
class GWService(pb2_grpc.NVMEGatewayServicer):
    ...
    def __init__(self, nvme_config):
        ...
        try:
            ipaddr = ipaddress.ip_address(gateway_addr.strip('[]'))
            if not ipaddr.is_unspecified:
                traddr = str(ipaddr)
        ...
        if not self.traddr:
            self.terminate("Failed to get gateway address")
    ...
    def restore_config(self):
        callbacks = {}
        callbacks[self.persistent_config.BDEV_PREFIX] = self.bdev_rbd_create
        callbacks[self.persistent_config.
                  SUBSYSTEM_PREFIX] = self.nvmf_create_subsystem
        callbacks[self.persistent_config.
    ...
    def bdev_rbd_create(self, request, context=None):
        ...
        if context:
            ...
            try:
                self.persistent_config.add_bdev(bdev_name, str(request))
            except Exception as ex:
    ...
    def nvmf_subsystem_add_host(self, request, context=None):
        ...
        try:
            if request.allow_any_host:  # Allow any host access to subsystem
                self.logger.info({
    ...
    def nvmf_subsystem_remove_host(self, request, context=None):
        ...
        try:
            if request.disable_allow_any_host:  # Disable allow any host access
                self.logger.info({
        ...
        if context:
            # Update persistent configuration
            try:
                if request.disable_allow_any_host:
    ...
    def nvmf_subsystem_add_listener(self, request, context=None):
        ...
        self.logger.info({
            f"Adding listener at {self.traddr} : {request.trsvcid} for {request.nqn}"
        })
    ...
    def nvmf_subsystem_remove_listener(self, request, context=None):
        ...
        self.logger.info(
            {f"Removing listener at {self.traddr} : {request.trsvcid} for {request.nqn}."})
