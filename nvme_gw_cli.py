...
class Parser:
    ...
    def cmd(self, args=[]):
        ...
        def decorator(func):
            ...
            mutex_group_added = False
            # Add specified arguments to the parser and set the function
            # attribute to point to the subcommand's associated function
            for arg in args:
                # Check args for mutually exclusive group
                if arg[1].pop("mutex", False):
                    if not mutex_group_added:
                        mutex_group = parser.add_mutually_exclusive_group(
                            required=True)
                        mutex_group_added = True
                    mutex_group.add_argument(*arg[0], **arg[1])
                else:
...
class GatewayClient:
    ...
    @cli.cmd([
        argument("-n", "--subnqn", help="Subsystem NQN", required=True),
        argument("-t", "--host", help="Host NQN", mutex=True),
        argument(
            "-a",
            "--any",
            help="Allow open host access to subsystem",
            default=False,
            action="store_true",
            mutex=True,
        )
    ...
    def add_host(self, args):
        ...
        if args.any or args.host == "*":
            allow_any_host = True

        try:
            create_req = pb2.subsystem_add_host_req(
            ...
            if allow_any_host:
                self.logger.info(
                    f"Any host access to {args.subnqn}: {ret.status}")
            else:
                self.logger.info(
                    f"Add host {args.host} access to {args.subnqn}: {ret.status}"
                )
        except Exception as error:
    ...
    @cli.cmd([
        ...
        argument(
            "-a",
            "--any",
            help="Disable open host access to subsystem",
            default=False,
            action="store_true",
            mutex=True,
        )
    ...
    def delete_host(self, args):
        ...
        if args.any or args.host == "*":
            disable_allow_any_host = True

        try:
            delete_req = pb2.host_delete_req(
                subsystem_nqn=args.subnqn,
                host_nqn=args.host,
                disable_allow_any_host=disable_allow_any_host)
            ret = self.stub.nvmf_subsystem_remove_host(delete_req)
            if disable_allow_any_host:
                self.logger.info(
                    f"Disable open host access in {args.subnqn}: {ret.status}")
            else:
                self.logger.info(
                    f"Remove host {args.host} access from {args.subnqn}: {ret.status}"
                )
        except Exception as error:
            self.logger.error(f"Failed to add host: \n {error}")
